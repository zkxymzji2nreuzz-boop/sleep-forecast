      ...
      <div className="container mx-auto max-w-screen-md px-4 py-10 sm:py-14 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <Card className="border-0 bg-transparent shadow-none">
          ...
          <CardContent className="space-y-4 text-sm text-foreground">
            ...
            {/* 3日目マイルストーン */}
            {isMilestone3 && (
              <div className="rounded-xl border border-primary/25 bg-primary/[0.07] p-4 text-center">
                <p className="text-sm font-semibold text-primary/80">3日連続！いいペースです</p>
                <p className="mt-1 text-xs text-muted-foreground">あと4日で気圧との相関グラフが解放されます</p>
              </div>
            )}
            ...
            {/* 気象コンテキスト */}
            <dl className="grid grid-cols-2 gap-x-4 gap-y-3 rounded-xl border border-border bg-card/50 p-4">
              <div>
                <dt className="text-[11px] uppercase tracking-wider text-muted-foreground">昨晩の眠り</dt>
                <dd className="mt-1 text-xl font-semibold text-foreground">{qualityLabel}</dd>
              </div>
              <div>
              ...
              <div>
                <dt className="text-[11px] uppercase tracking-wider text-muted-foreground">気温</dt>
                <dd className="mt-1 text-xl font-semibold tabular-nums text-foreground">{w.temperatureC.toFixed(1)}°C</dd>
              </div>
              <div>
                <dt className="text-[11px] uppercase tracking-wider text-muted-foreground">湿度</dt>
                <dd className="mt-1 text-xl font-semibold tabular-nums text-foreground">{w.humidity}%</dd>
              </div>
            </dl>

            {/* 気象ナラティブ（2件目以降のみ表示） */}
            {weatherNarrative && (
              <div className="flex items-start gap-2.5 rounded-xl border border-border bg-card/50 px-4 py-3">
                <Moon className="mt-0.5 h-4 w-4 shrink-0 text-primary/80" aria-hidden="true" />
                <p className="text-xs leading-relaxed text-muted-foreground">{weatherNarrative}</p>
              </div>
            )}
            ...
            {recordCountAfterSave > 0 && recordCountAfterSave < 7 && (
              <div className="rounded-xl border border-border bg-card/50 px-4 py-3">
                <div className="mb-2 flex items-center justify-between text-xs">
                  <span className="text-muted-foreground">相関分析まで</span>
                  <span className="font-semibold text-primary/80">{recordCountAfterSave} / 7日</span>
                </div>
                ...
                <p className="mt-1.5 text-[11px] text-muted-foreground">あと {remaining} 日で気象との相関グラフが解放されます</p>
              </div>
            )}
            ...
            <div className="flex flex-col gap-2 pt-1">
              ...
              {recordCountAfterSave >= 7 ? (
                <>
                  <Button asChild className="w-full bg-gradient-to-r from-primary to-primary/60 text-white hover:opacity-90">
                    <Link href="/dashboard">
                      <BarChart3 className="mr-2 h-4 w-4" aria-hidden="true" />
                      ダッシュボードで傾向を確認
                    </Link>
                  </Button>
                  <Button asChild variant="outline" className="w-full border-border text-muted-foreground hover:border-primary/25 hover:text-primary/80">
                    <Link href="/">
              ...
              <Button type="button" variant="ghost" className="w-full text-muted-foreground/60 hover:bg-card/50 hover:text-muted-foreground" onClick={handleEditAgain}>
  ...
  return (
    <div className="container mx-auto max-w-screen-md px-4 py-8 pb-24 sm:py-12 sm:pb-0">
      ...
      <div className="mb-6 text-center">
        <div className="mb-4 flex items-center justify-center gap-4 text-xs tabular-nums text-muted-foreground">
      ...
      {todayWeather === "loading" ? (
        <div className="mb-4 flex items-center gap-2 rounded-xl border border-border bg-card/50 px-4 py-3">
          ...
          <span className="text-xs text-muted-foreground">気象データを取得中…</span>
        </div>
      ) : todayWeather ? (
        <div className="mb-4 rounded-xl border border-border bg-card/50 px-4 py-3">
          <p className="mb-2 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">今日の気象</p>
          <div className="flex flex-wrap gap-x-5 gap-y-1.5 tabular-nums text-sm">
            <span className="flex items-baseline gap-1.5">
              <span className="text-[10px] text-muted-foreground">気圧</span>
            ...
            <span className="flex items-baseline gap-1.5">
              <span className="text-[10px] text-muted-foreground">気温</span>
              <span className="text-foreground">{todayWeather.temperatureC.toFixed(1)}°C</span>
            </span>
            <span className="flex items-baseline gap-1.5">
              <span className="text-[10px] text-muted-foreground">湿度</span>
              <span className="text-foreground">{todayWeather.humidity}%</span>
            </span>
          </div>
          {todayWeather.pressureDeltaHpa <= -3 && (
          ...
          <CardContent className="space-y-8 p-0 sm:p-2">
                ...
                {QUALITY_OPTIONS.map((opt) => {
                  ...
                  return (
                    <button
                      ...
                      className={[
                        "flex min-h-[88px] min-w-[44px] flex-col items-center justify-center gap-2 rounded-xl bg-card/50 p-3 text-muted-foreground transition-colors hover:bg-card/70 hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary",
                        selected ? "bg-primary/[0.07] text-foreground ring-1 ring-inset ring-primary/60 shadow-[0_0_24px_-8px_rgba(99,102,241,0.55)]" : "",
                      ].join(" ")}
                    >
                      ...
                      {selected ? <span className="block h-0.5 w-6 rounded-full bg-primary" aria-hidden="true" /> : null}
                    </button>
                  );
                })}
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                <Label htmlFor="bedtime" className="flex items-center text-sm font-medium text-foreground">
                  <Moon className="mr-1 h-3.5 w-3.5 text-muted-foreground" aria-hidden="true" />
                  就寝時刻 (任意)
                </Label>
              ...
              <div className="space-y-2">
                <Label htmlFor="wakeTime" className="flex items-center text-sm font-medium text-foreground">
                  <Sunrise className="mr-1 h-3.5 w-3.5 text-muted-foreground" aria-hidden="true" />
                  起床時刻 (任意)
                </Label>
            ...
            {/* 送信ボタン + スキップリンク */}
            <div className="sticky bottom-0 -mx-4 mt-2 border-t border-border bg-background/90 p-4 pb-[env(safe-area-inset-bottom)] backdrop-blur sm:static sm:mx-0 sm:border-0 sm:bg-transparent sm:p-0 sm:backdrop-blur-none">
              <Button type="submit" className="h-12 w-full bg-primary text-base font-semibold text-white hover:opacity-90 disabled:opacity-50" disabled={submitDisabled}>
                {isSubmitting ? (
              ...
              <div className="mt-6 text-center">
                <Link href="/" className="text-sm text-muted-foreground underline-offset-4 hover:text-muted-foreground/70 hover:underline">
                  今日はスキップ
                </Link>
              </div>
            </div>
