...
RecordForm は 1 / 3 / 5 の 3 択のみ提供する（2・4 は入力不可）。
チャート・集計は 1/3/5 の 3 点に基づいて表示する。
内部型 `SleepQuality = 1|2|3|4|5` は予測計算の中間値に使用するが、ユーザー入力値は常に 1/3/5 のいずれか。

| value | 絵文字 | ラベル | aria-label | 備考 |
|---|---|---|---|---|
| 1 | 😴 | 眠れなかった | `睡眠品質 1 眠れなかった` | 全く眠れなかった / 何度も中途覚醒 |
| 3 | 🙂 | なんとか眠れた | `睡眠品質 3 なんとか眠れた` | 平均的 / やや浅い |
| 5 | 🌟 | よく眠れた | `睡眠品質 5 よく眠れた` | スッキリ目覚めた / 過去最高クラス |
...
- `quality`: `[3, 1, 3, 3, 5, 1, 3, 3, 3, 1, 3, 5, 3, 3, 1, 3, 3, 5, 3, 1, 3, 3, 3, 1, 3, 5, 3, 3, 3, 3]` の固定配列 (先頭が最新、値は 1/3/5 のみ)
...
- Y 軸: 1〜5 固定、整数目盛り (`stepSize: 1`)、ラベル: 1=`眠れなかった`、3=`なんとか眠れた`、5=`よく眠れた`、2・4 は空文字を `callback` で返す
...
- 点色: quality 値で色分け（3択のみ）— 1: `#f87171`、3: `#facc15`、5: `#4ade80`。各点に `pointBackgroundColor` を配列で指定
...
- Tooltip: `「気圧変化: -3.7 hPa — 品質: なんとか眠れた」` 形式（quality 1=眠れなかった / 3=なんとか眠れた / 5=よく眠れた）
